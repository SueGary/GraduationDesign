...
{
    /// <summary>
    /// 对象名称：“空白模块模块”删除界面控件（用户界面层）
    /// 对象说明：该界面用于删除“个人信息”对象并提交至数据库，删除操作成功后返回列表界面。
    /// 作者姓名：苏嘉立
    ...
    public partial class Delete : UserControl
    {
        ...
        /// </summary>
        public static void LoadDataById(string gRXXId)
        {
            if (instance == null)
            {
                instance = new Delete();
            }
            instance.gRXX = GRXXBLL.GetDataBySerialId(gRXXId);
            if (instance.gRXX == null)
        ...
        {
            GRXXBLL.Delete(gRXX.SerialId.Value); // 调用“业务逻辑层”的方法，删除关联对象并更新至数据库。
            FormSysMessage.ShowSuccessMsg("“个人信息”删除成功，单击“确定”按钮返回信息列表。");
